...
pub fn event_loop(
    mut inotify_state: InotifyState,
            ...
            Ok(events) => {
                for event in events {
                    ...
                    if event.mask.contains(AddWatchFlags::IN_IGNORED) {
                        Logger::error(format!(
                            "IN_IGNORE received on {event:?}! File will not be watched."
                        ));
                        continue;
                    }
                    if event.mask.contains(AddWatchFlags::IN_Q_OVERFLOW) {
                        Logger::error(
                            "Inotify event queue is overflowed. Some events may be dropped",
                        );
                    }
                    ...
                    match command.spawn() {
                        ...
                        Err(err) => {
                            Logger::error(format!(
                                "Error spawning a child script {script:?}: {err}"
                            ));
                        }
                    };
                }
    ...
    loop {
        match signal_fd.read_signal() {
            Ok(signal) => match signal {
                Some(signal) => match Signal::try_from(signal.ssi_signo as i32) {
                    Ok(Signal::SIGHUP) => {
...
pub fn reload_cfg(inotify_state: &mut InotifyState, cfg: &mut Cfg) {
    match Cfg::init(&cfg.cli_config_path) {
        ...
        Err(err) => Logger::error(format!(
            "Error parsing new config, reloading with the previous one: {err}"
        )),
    }
...
pub fn inotify_fill_from_cfg(inotify_state: &mut InotifyState, cfg: &Cfg) {
    for entry in &cfg.entry {
        ...
        let objs_to_watch =
            iter::once(entry.path.as_path()).chain(extra.iter().map(PathBuf::as_path));

        for obj in objs_to_watch {
            match inotify_state
                .inotify_fd
                .add_watch(obj, entry.events | AddWatchFlags::IN_DONT_FOLLOW)
            {
                Ok(wd) => {
                    inotify_state
                        .wd_to_script
                        .insert(wd, Rc::clone(&entry.script));
...
pub fn init_epoll(inotify_state: &InotifyState, signal_fd: &SignalFd) -> Result<Epoll, Errno> {
    ...
    epoll_fd.add(
        &inotify_state.inotify_fd,
        EpollEvent::new(EpollFlags::EPOLLIN, INOTIFY_EPOLL_TOKEN),
    )?;
    epoll_fd.add(
        signal_fd,
        EpollEvent::new(EpollFlags::EPOLLIN, SIGNAL_EPOLL_TOKEN),
    )?;
...
mod tests {
    ...
    fn nonexistent_path_returns_none() {
        init_test_logger();
        let result = recursive_dir_walk(Path::new(
            "/jhlkasdghasdgfkjha/asdgikahsdgkljasdg/pqoruoiqwpro/oooooooo",
        ));
